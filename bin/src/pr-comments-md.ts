...
function usage(): void {
  console.error(
    "Usage: pr-comments-md <https://github.com/<owner>/<repo>/pull/<number>>",
  )
}
...
function parsePrUrl(raw: string): {
  owner: string
  repo: string
  number: number
...
function lineDisplay(thread: ReviewThread): string {
  ...
  if (thread.originalLine !== null) {
    if (
      ...
      return `original ${thread.originalStartLine}-${thread.originalLine}`
    }
    return `original ${thread.originalLine}`
  }
  return "unknown"
}
...
function gatherSelectableThreads(threads: ReviewThread[]): SelectableThread[] {
  ...
  for (const thread of threads) {
    ...
    const comments = [...thread.comments.nodes].sort(
...
async function selectThreadsInteractive(
  items: SelectableThread[],
): Promise<SelectableThread[]> {
...
function formatMarkdown(
  ...
  for (const thread of selected) {
    ...
    for (const [index, comment] of thread.comments.entries()) {
      const id =
        comment.fullDatabaseId !== null ? comment.fullDatabaseId : comment.id
...
async function main(): Promise<void> {
  ...
  await $`printf '%s' ${markdown} | pbcopy`.quiet()
